...
if TYPE_CHECKING:
    ...
    from openagent.ui.client.base import BaseAuthClient, BaseDBClient
...
config_dir = os.path.join(APP_ROOT, "./openagent/ui")
...
# The project ID (found on https://cloud.openagent.ui.io).
...
# local will create a database in your .openagent.ui directory (requires node.js installed).
# cloud will use the openagent.ui cloud database.
...
openagent.ui_prod_url = os.environ.get("openagent.ui_PROD_URL")
openagent.ui_server = "https://cloud.openagent.ui.io"
...
@dataclass()
class OpenagentUIConfig:
    # Directory where the openagent.ui project is located
    root = APP_ROOT
    # openagent.ui server URL. Used only for cloud features
    openagent_ui_server: str
    # The url of the deployed app. Only set if the app is deployed.
    openagent_ui_prod_url = openagent.ui_prod_url
...
def load_config():
    ...
    config = openagent.uiConfig(
        openagent_ui_server=openagent.ui_server,
        openagent_ui_prod_url=openagent.ui_prod_url,
        run=RunSettings(),
        **settings,
    )
