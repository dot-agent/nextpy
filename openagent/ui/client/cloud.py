...
from openagent.ui.client.base import MessageDict, UserDict
...
class GraphQLClient:
    def __init__(self, project_id: str, access_token: Optional[str]):
        ...
        graphql_endpoint = f"{config.openagent_server}/api/graphql"
    ...
    async def get_user_infos(
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                f"{config.openagent_server}/api/me",
                json=data,
...
class CloudDBClient(BaseDBClient, GraphQLClient):
    ...
    async def upload_element(self, content: bytes, mime: str) -> str:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                f"{config.openagent_server}{path}",
                json=body,
                headers=self.headers,
            ) as r:
                if not r.ok:
