...
class OpenagentUITelemetry:
    ...
    def _build_resource(
        ...
        if config.openagent_prod_url:
            host_name = config.openagent_prod_url
    ...
    def configure_tracer(self):
        ...
        tracer = get_tracer("openagent", __version__)
        return tracer
...
openagent_telemetry = OpenagentUITelemetry()


def trace_event(event_name):
    if config.project.enable_telemetry:
        with openagent_telemetry.tracer.start_as_current_span(
