...
from openagent.ui.client.utils import (
    get_auth_client_from_request,
    get_db_client_from_request,
)
from openagent.ui.config import DEFAULT_HOST, config, load_module, reload_config
from openagent.ui.logger import logger
from openagent.ui.markdown import get_markdown_str
from openagent.ui.telemetry import trace_event
from openagent.ui.types import (
    CompletionRequest,
...
async def lifespan(app: FastAPI):
    ...
    if config.run.watch:
        ...
        async def watch_files_for_changes():
            extensions = [".py"]
            files = ["openagent.ui.md", "config.toml"]
...
app.mount(
    "/assets",
    StaticFiles(packages=[("openagent", os.path.join(build_dir, "assets"))]),
    name="assets",
)
...
def get_html_template():
    ...
    default_url = "https://github.com/zen-op/openagent-prompt"
...
async def project_settings():
    ...
    return JSONResponse(
        content={
            "openagentServer": config.openagent_server,
            "prod": bool(config.openagent_prod_url),
...
import openagent.ui.socket  # noqa
