...
if TYPE_CHECKING:
    ...
    from openagent.gui.client.base import BaseAuthClient, BaseDBClient
...
config_dir = os.path.join(APP_ROOT, ".gui")
...
# The project ID (found on https://cloud.gui.io).
...
# local will create a database in your .gui directory (requires node.js installed).
# cloud will use the Gui cloud database.
...
gui_prod_url = os.environ.get("GUI_PROD_URL")
gui_server = "https://cloud.dotagent.ai"
...
@dataclass()
class GuiConfig:
    # Directory where the Gui project is located
    root = APP_ROOT
    # Gui server URL. Used only for cloud features
    gui_server: str
    # The url of the deployed app. Only set if the app is deployed.
    gui_prod_url = gui_prod_url
...
def load_module(target: str):
    ...
    spec = util.spec_from_file_location(str(target), str(target))
