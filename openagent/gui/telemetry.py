...
class GuiTelemetry:
    ...
    def _build_resource(
        ...
        if config.gui_prod_url:
            host_name = config.gui_prod_url
    ...
    def configure_tracer(self):
        ...
        tracer = get_tracer("gui", __version__)
        return tracer
...
gui_telemetry = GuiTelemetry()


def trace_event(event_name):
    if config.project.enable_telemetry:
        with gui_telemetry.tracer.start_as_current_span(
