...
from openagent.gui.cli.auth import get_access_token
...
def upload_tar_gz_archive(access_token: str, archive_path: str):
    url = f"{config.gui_server}/api/upload/deployment"
...
def deploy(target: str):
    if not config.project.id:
        raise Exception(
            "Project id not set in config. A project id is mandatory to deploy a gui app."
        )
    ...
    with tempfile.TemporaryDirectory() as tempdir:
        archive_path = os.path.join(tempdir, "gui.tar.gz")
