...
from openagent.gui.client.base import MessageDict
from openagent.gui.client.cloud import CloudAuthClient
from openagent.gui.client.utils import get_auth_client, get_db_client
from openagent.gui.config import config
from openagent.gui.context import emitter_var, loop_var
from openagent.gui.emitter import GuiEmitter
from openagent.gui.logger import logger
from openagent.gui.message import ErrorMessage, Message
from openagent.gui.server import socket
from openagent.gui.session import Session
from openagent.gui.telemetry import trace_event
from openagent.gui.user_session import user_sessions


def load_gui_initial_headers(http_cookie):
    cookie = SimpleCookie(http_cookie)
    cookie_string = ""
    initial_headers = cookie.get("gui-initial-headers")
    if (initial_headers):
        cookie_string = initial_headers.value
    if cookie_string:
        try:
            gui_initial_headers = json.loads(cookie_string)
        except ValueError:
            gui_initial_headers = {}
    else:
        gui_initial_headers = {}

    return gui_initial_headers
...
async def connect(sid, environ, auth):
    ...
    session_id = environ.get("HTTP_X_GUI_SESSION_ID")
    if restore_existing_session(sid, session_id, emit_fn, ask_user_fn):
        return True

    request_headers = load_gui_initial_headers(environ.get("HTTP_COOKIE"))
...
async def connection_successful(sid):
    ...
    emitter_var.set(GuiEmitter(session))
