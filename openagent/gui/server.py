...
from openagent.gui.client.utils import (
    get_auth_client_from_request,
    get_db_client_from_request,
)
from openagent.gui.config import DEFAULT_HOST, config, load_module, reload_config
from openagent.gui.logger import logger
from openagent.gui.markdown import get_markdown_str
from openagent.gui.telemetry import trace_event
from openagent.gui.types import (
    CompletionRequest,
...
# For the index.html
index_dir = os.path.join(root_dir, "frontend")  # Change src to frontend for the index.html

# For assets
build_dir = os.path.join(root_dir, "frontend", "src", "assets")


app = FastAPI(lifespan=lifespan)

# Correct the path for the public directory
app.mount("/public", StaticFiles(directory=os.path.join(root_dir, "public"), check_dir=False), name="public")

# Correct the path for the assets directory
...
def get_html_template():
    ...
    default_url = "https://github.com/dot-agent/openagent"
    ...
    <meta property="og:image" content="https://gui-cloud.s3.eu-west-3.amazonaws.com/logo/gui_banner.png">
    ...
    index_html_file_path = os.path.join(index_dir, "index.html")
...
async def project_settings():
    ...
    return JSONResponse(
        content={
            "guiServer": config.gui_server,
            "prod": bool(config.gui_prod_url),
...
async def get_favicon():
    favicon_path = os.path.join(root_dir, "frontend", "public", "favicon.svg")
...
import openagent.gui.socket  # noqa