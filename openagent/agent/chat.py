...
class ChatAgent(BaseAgent):
    def __init__(self,
        ...
        self.prompt_template = prompt_template if prompt_template is not None else self.default_prompt_template

        self.output_key = output_key
        self.compiler = compiler(llm = self.llm, template = self.prompt_template, caching=caching, memory = self.memory)
