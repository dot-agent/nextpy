...
def _get_parent_imports(func):
    _imports = {"nextpy.backend.vars": ["Var"]}
...
def generate_init():
    ...
    from nextpy import _MAPPING  # type: ignore
    ...
    with open("nextpy/__init__.pyi", "w") as pyi_file:
        pyi_file.writelines("\n".join(imports))
...
if __name__ == "__main__":
    ...
    targets = sys.argv[1:] if len(sys.argv) > 1 else ["nextpy/frontend/components"]
