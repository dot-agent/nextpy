...
def VarOperations():
    ...
    def index():
        return xt.vstack(
            ...
            xt.text(xt.Var.range(2, 5).join(","), id="list_join_range1"),
            xt.text(xt.Var.range(2, 10, 2).join(","), id="list_join_range2"),
            xt.text(xt.Var.range(5, 0, -1).join(","), id="list_join_range3"),
            xt.text(xt.Var.range(0, 3).join(","), id="list_join_range4"),
            xt.box(
                xt.foreach(
                    xt.Var.range(0, 2), lambda x: xt.text(VarOperationState.list1[x])
                ),
                id="foreach_list_arg",
            ),
            xt.box(
                xt.foreach(
                    xt.Var.range(0, 2),
                    lambda x, ix: xt.text(VarOperationState.list1[ix]),
                ),
                id="foreach_list_ix",
            ),
            xt.box(
                xt.foreach(
                    xt.Var.create_safe(list(range(0, 3))).to(list[int]),
                    lambda x: xt.foreach(
                        xt.Var.range(x),
                        lambda y: xt.text(VarOperationState.list1[y]),
                    ),
                ),
