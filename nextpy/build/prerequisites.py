...
def get_app(reload: bool = False) -> ModuleType:
    ...
    if not config.app_name:
        raise RuntimeError(
            "Cannot get the app module because `app_name` is not set in xtconfig! "
            "If this error occurs in a nextpy test case, ensure that `get_app` is mocked."
        )
...
def get_default_app_name() -> str:
    ...
    Raises:
        Exit: if the app directory name is nextpy.
    """
    app_name = os.getcwd().split(os.path.sep)[-1].replace("-", "_")

    # Make sure the app is not named "nextpy".
    if app_name == constants.Nextpy.MODULE_NAME:
        console.error(
            f"The app directory cannot be named [bold]{constants.Nextpy.MODULE_NAME}[/bold]."
        )
        raise typer.Exit(1)
    ...
    try:
        ...
        with open(fp, "r", encoding=encoding) as f:
            for req in f.readlines():
                # Check if we have a package name that is nextpy
                if re.match(r"^nextpy[^a-zA-Z0-9]", req):
        ...
        with open(fp, "a", encoding=encoding) as f:
            ...
            f.write(
                f"{preceding_newline}{constants.RequirementsTxt.DEFAULTS_STUB}{constants.Nextpy.VERSION}\n"
            )
    except Exception:
...
def get_project_hash() -> int | None:
    """Get the project hash from the nextpy.json file if the file exists.
    ...
    with open(constants.Nextpy.JSON, "r") as file:
        data = json.load(file)
...
def initialize_web_directory():
    ...
    # Re-use the hash if one is already created, so we don't over-write it when running nextpy init
    ...
    # Initialize the nextpy json file.
    init_nextpy_json(project_hash=project_hash)
...
def init_nextpy_json(project_hash: int | None):
    """Write the hash of the Nextpy project to a NEXTPY_JSON.

    Re-use the hash if one is already created, therefore do not
    overwrite it every time we run the nextpy init command
    .
    ...
    nextpy_json = {
        "version": constants.Nextpy.VERSION,
        "project_hash": project_hash,
    }
    path_ops.update_json_file(constants.Nextpy.JSON, nextpy_json)


def update_next_config(export=False):
    """Update Next.js config from Nextpy config.

    Args:
        export: if the method run during nextpy export.
    """
...
def install_node():
    """Install fnm and nodejs for use by Nextpy.
...
def install_bun():
    ...
    if unzip_path is None:
        raise FileNotFoundError("Nextpy requires unzip to be installed.")
...
def check_initialized(frontend: bool = True):
    ...
    # Check presence of the application configuration file
    has_config = os.path.exists(constants.Config.FILE)

    # Check for presence of the Nextpy core directory
    ...
    has_nextpy_dir = not frontend or os.path.exists(constants.Nextpy.DIR) 

    # Check for the existence of the frontend web directory   
    has_web_dir = not frontend or os.path.exists(constants.Dirs.WEB) 

   
    # Check if the app is initialized.
    if not (has_config and has_nextpy_dir and has_web_dir):
        console.error(
            f"The app is not initialized. Run [bold]{constants.Nextpy.MODULE_NAME} init[/bold] first."
        )
        raise typer.Exit(1)
...
def is_latest_template() -> bool:
    ...
    if not os.path.exists(constants.Nextpy.JSON):
        return False
    with open(constants.Nextpy.JSON) as f:  # type: ignore
        app_version = json.load(f)["version"]
    return app_version == constants.Nextpy.VERSION
...
def validate_bun():
    ...
    if bun_path != constants.Bun.DEFAULT_PATH:
        ...
        if not bun_version:
            ...
            console.error(
                f"Nextpy requires bun version {constants.Bun.VERSION} or higher to run, but the detected version is "
...
def validate_frontend_dependencies(init=True):
    ...
    Args:
        init: whether running `nextpy init`
    ...
    if not init:
        ...
        # `nextpy init` will install the deps anyway(if applied).
        ...
        if not check_node_version():
            ...
            console.error(
                f"Nextpy requires node version {constants.Node.MIN_VERSION} or higher to run, but the detected version is {node_version}",
            )
            raise typer.Exit(1)
    ...
    if init:
        # we only need bun for package install on `nextpy init`.
        validate_bun()
...
def check_db_initialized() -> bool:
    ...
    if get_config().db_url is not None and not Path(constants.ALEMBIC_CONFIG).exists():
        console.error(
            "Database is not initialized. Run [bold]nextpy db init[/bold] first."
        )
        return False
    return True
    ...
    with model.Model.get_db_engine().connect() as connection:
        try:
            if model.Model.alembic_autogenerate(
                ...
                console.error(
                    "Detected database schema changes. Run [bold]nextpy db makemigrations[/bold] "
...
def prompt_for_template() -> constants.Templates.Kind:
    ...
    console.print("blank (https://blank-template.nextpy.app) - A minimal template.")
    console.print(
        "base (https://base-template.nextpy.app) - A base template with a sidebar to navigate pages."
    )
    console.print("")
...
def migrate_to_nextpy():
    """Migration from Reflex to Nextpy."""
    ...
    action = console.ask(
        "Reflex project detected. Automatically upgrade to Nextpy?",
        choices=["y", "n"],
    )
    if action == "n":
        return

    # Rename rxconfig to xtconfig.
    console.log(
    ...
    updates = {
        "Reflex": "Nextpy",
        "reflex as xt": "nextpy as xt",
        "reflex.dev": "nextpy.org",
        "reflex": "nextpy",
        "rx.": "xt.",
        "rxconfig": "xtconfig",
    }
