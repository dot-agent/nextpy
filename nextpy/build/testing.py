...
import nextpy
import nextpy.cli
import nextpy.build.build
import nextpy.build.exec
import nextpy.build.prerequisites
import nextpy.build.processes
from nextpy.backend.state import BaseState, State, StateManagerMemory, StateManagerRedis
...
class AppHarness:
    ...
    def _initialize_app(self):
        ...
        with chdir(self.app_path):
            ...
            # self.app_module.app.
            self.app_module = nextpy.build.prerequisites.get_app(reload=True)
    ...
    def _start_frontend(self):
        # Set up the frontend.
        with chdir(self.app_path):
            config = nextpy.build.config.get_config()
            config.api_url = "http://{0}:{1}".format(
                *self._poll_for_servers().getsockname(),
            )
            nextpy.build.build.setup_frontend(self.app_path)

        # Start the frontend.
        self.frontend_process = nextpy.build.processes.new_process(
            [nextpy.build.prerequisites.get_package_manager(), "run", "dev"],
...
class AppHarnessProd(AppHarness):
    ...
    def _run_frontend(self):
        web_root = self.app_path / nextpy.constants.Dirs.WEB_STATIC
        error_page_map = {
    ...
    def _start_backend(self):
        ...
        self.backend = uvicorn.Server(
            uvicorn.Config(
                ...
                host="127.0.0.1",
                port=0,
                workers=nextpy.build.processes.get_num_workers(),
            ),
        )
        self.backend.shutdown = self._get_backend_shutdown_handler()
        self.backend_thread = threading.Thread(target=self.backend.run)
