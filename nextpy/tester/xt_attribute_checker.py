...
class NextpyAttributeChecker(BaseChecker):
    ...
    def visit_attribute(self, node: astroid.Attribute) -> None:
        if isinstance(node.expr, astroid.Name) and (node.expr.name == 'nextpy' or node.expr.name == 'xt'):
            if node.attrname not in self.allowed_attributes:
                closest_match = self.find_closest_attribute(node.attrname)
                self.add_message('non-existent-nextpy-attribute', node=node, args=(node.attrname, closest_match))


    def find_closest_attribute(self, attr_name: str) -> str:
        closest_matches = difflib.get_close_matches(attr_name, self.allowed_attributes, n=1)
