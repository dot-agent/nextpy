...
from nextpy.backend.admin import AdminDash
from nextpy.backend.event import Event, EventHandler, EventSpec
from nextpy.backend.middleware import HydrateMiddleware, Middleware
...
from nextpy.interfaces.web.react_components import connection_modal
from nextpy.interfaces.web.react_components.base.app_wrap import AppWrap
from nextpy.interfaces.web.react_components.base.fragment import Fragment
from nextpy.interfaces.web.react_components.component import Component, ComponentStyle
from nextpy.interfaces.web.react_components.core.client_side_routing import (
    Default404Page,
    wait_for_client_redirect,
)
from nextpy.interfaces.web.react_components.radix import themes
...
class App(Base):
    ...
    def __init__(self, *args, **kwargs):
        ...
        Raises:
            ValueError: If the event namespace is not provided in the config.
                        Also, if there are multiple client subclasses of xt.State(Subclasses of xt.State should consist
        ...
        # Special case to allow test cases have multiple subclasses of xt.BaseState.
        if not is_testing_env:
            # Only one Base State class is allowed.
            if len(state_subclasses) > 1:
                raise ValueError(
                    "xt.BaseState cannot be subclassed multiple times. use xt.State instead"
                )
        ...
        try:
            ...
            if "BaseVar" in message or "ComputedVar" in message:
                raise TypeError(
                    ...
                    "See the var operation docs here: https://nextpy.org/docs/state/vars/#var-operations"
                ) from e
            raise e
        ...
        if admin_dash and admin_dash.models:
            ...
            admin = (
                ...
                if admin_dash.admin
                    ...
                    title="Nextpy Admin Dashboard",
                    logo_url="https://nextpy.org/Nextpy.svg",
                )
            )
    ...
    def get_frontend_packages(self, imports: Dict[str, set[ReactImportVar]]):
...
def upload(app: App):
    ...
    async def upload_file(request: Request, files: List[UploadFile]):
        ...
        """
        token = request.headers.get("nextpy-client-token")
        handler = request.headers.get("nextpy-event-handler")

        if not token or not handler:
            raise HTTPException(
                status_code=400,
                detail="Missing nextpy-client-token or nextpy-event-handler header.",
            )
        ...
        if isinstance(func, EventHandler):
            if func.is_background:
                raise TypeError(
                    f"@xt.background is not supported for upload handler `{handler}`.",
                )
            func = func.fn
...
# def SingletonApp(*args, **kwargs):
#     if hasattr(SingletonApp, "app"):
#         return SingletonApp.app
#     else:
#         SingletonApp.app = App(*args, **kwargs)
#     return SingletonApp.app
