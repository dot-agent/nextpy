...
class ChakraComponent(Component):
    ...
    def _get_dependencies_imports(cls) -> imports.ImportDict:
        ...
        """
        return {
            dep: [imports.ReactImportVar(tag=None, render=False)]
            for dep in [
                "@chakra-ui/system@2.5.7",
                "focus-visible@5.2.0",
                "framer-motion@10.16.4",
...
class ChakraProvider(ChakraComponent):
    ...
    def _get_imports(self) -> imports.ImportDict:
        ...
        _imports.setdefault(self.__fields__["library"].default, []).append(
            imports.ReactImportVar(tag="extendTheme", is_default=False),
        )
        _imports.setdefault("/utils/theme.js", []).append(
            imports.ReactImportVar(tag="theme", is_default=True),
        )
        _imports.setdefault(Global.__fields__["library"].default, []).append(
            imports.ReactImportVar(tag="css", is_default=False),
        )
        return _imports
