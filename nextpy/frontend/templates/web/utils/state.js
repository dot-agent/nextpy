/**
 * Nextpy Web App State Management
 * 
 * Overview:
 * - Manages state, events, and communication in Nextpy (Next.js + Python) web applications.
 * 
 * Functionalities:
 * - Token & UUID: For session tracking.
 * - WebSockets: Real-time, bidirectional communication.
 * - Storage Utils: Interactions with client-side storage.
 * - Reducers: Apply updates to the app's state.
 * - Frontend Utils: Various utility functions for frontend interactions.
 * - React Hooks: Integrate React's features into custom hooks.
 * 
 * Concepts:
 * - Context & State: React's 'context' allows global access to data (like 'state') across components. 
 *   'State' represents data that might change and affect the app's UI. Together, they enable dynamic updates.
 * 
 */
...
export const uploadFiles = async (handler, files, upload_id, on_upload_progress, socket) => {
  ...
  const config = {
    headers: {
      "Nextpy-Client-Token": getToken(),
      "Nextpy-Event-Handler": handler,
    },
