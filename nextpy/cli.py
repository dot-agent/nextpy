"""Nextpy CLI to create, run, and deploy apps."""
...
def version(value: bool):
    ...
    """
    if value:
        console.print(constants.Nextpy.VERSION)
        raise typer.Exit()
...
def _run(
    ...
    from nextpy.build import build, exec, prerequisites, processes

    # Set the log level.
    console.set_log_level(loglevel)

    # Set env mode in the environment
    os.environ["NEXTPY_ENV_MODE"] = env.value
...
def export(
    ...
    from nextpy.build import build, exec, prerequisites
...
def login(
    ...
    """Authenticate with Nextpy hosting service."""
    from nextpy.backend.deploy import hosting
...
def logout(
    ...
    """Log out of access to Nextpy hosting service."""
    from nextpy.backend.deploy import hosting
...
def db_init():
    ...
    if Path(constants.ALEMBIC_CONFIG).exists():
        console.error(
            ...
            "[bold]nextpy db makemigrations[/bold] to create schema change "
            "scripts and [bold]nextpy db migrate[/bold] to apply migrations "
...
def deploy(
    ...
    """Deploy the app to the Nextpy hosting service."""
    from nextpy.backend.deploy import hosting
    from nextpy.build import prerequisites
    ...
    if server_report_deploy_success is None:
        ...
        console.error(
            f"Check the server logs using `nextpy deployments build-logs {key}`"
        )
        raise typer.Exit(1)
...
def hello(
    ...
    """Run the hello app."""
    # Open the hello app in a terminal.
    # webbrowser.open("https://hello.nextpy.org")

    # Later: open the hello app locally.
    with tempfile.TemporaryDirectory() as tmp_dir:
        os.chdir(tmp_dir)
        _init(
            name="nextpy_hello",
            template=constants.Templates.Kind.HELLO,
...
@cli.command()
def install(
    extras: str = typer.Argument(..., help="The name of the extras group to install.")
):
    """Install extra dependencies defined in pyproject.toml."""
    valid_extras = ["ams", "ml"]  # List other valid extras if needed
    if extras not in valid_extras:
        console.error(f"No extras group named '{extras}' found.")
        raise typer.Exit(1)

    console.print(f"Installing '{extras}' group of extra dependencies...")
    result = subprocess.run(
        ["poetry", "install", "-E", extras], capture_output=True, text=True
    )
