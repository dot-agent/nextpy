...
class Expander(ChakraComponent):
    """The wrapper that uses cloneElement to pass props to AccordionItem children."""
    ...
    def create(
        ...
        **props
        ...
        if len(children) == 0:
            ...
            for label, panel in items:
                if icon_pos == "right":
                    button = ExpanderButton.create(label, ExpanderIcon.create())
                elif icon_pos == "left":
                    button = ExpanderButton.create(ExpanderIcon.create(), label)
                else:
                    button = ExpanderButton.create(label)

                children.append(
                    ExpanderItem.create(
                        button,
                        ExpanderPanel.create(panel),
                    )
                )
...
class ExpanderItem(ChakraComponent):
    """A single accordion item."""
...
class ExpanderButton(ChakraComponent):
    """The button that toggles the expand/collapse state of the accordion item. This button must be wrapped in an element with role heading."""

    tag = "AccordionButton"


class ExpanderPanel(ChakraComponent):
    """The container for the details to be revealed."""

    tag = "AccordionPanel"


class ExpanderIcon(ChakraComponent):
    """A chevron-down icon that rotates based on the expanded/collapsed state."""
