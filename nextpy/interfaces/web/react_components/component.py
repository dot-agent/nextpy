...
from nextpy.interfaces.web.react_components.tags import Tag
...
class BaseComponent(Base, ABC):
    """The base class for all Nextpy components.

    This is something that can be rendered as a Component via the Nextpy compiler.
    """
...
class Component(BaseComponent, ABC):
    ...
    def create(cls, *children, **props) -> Component:
        ...
        for child in children:
            ...
            if not types._isinstance(child, ComponentChild):
                raise TypeError(
                    "Children of Nextpy components must be other components, "
    ...
    def _get_hooks_imports(self) -> imports.ImportDict:
        ...
        if self._get_mount_lifecycle_hook():
            ...
            _imports.setdefault("react", set()).add(ReactImportVar(tag="useEffect"))

        if self._get_special_hooks():
            # Handle additional internal hooks (autofocus, etc).
            _imports.setdefault("react", set()).update(
                {
                    ReactImportVar(tag="useRef"),
                    ReactImportVar(tag="useEffect"),
                },
            )
        return _imports
    ...
    def import_var(self):
        ...
        return ReactImportVar(tag=tag, is_default=self.is_default, alias=alias)
...
class NoSSRComponent(Component):
    ...
    def _get_imports(self) -> imports.ImportDict:
        ...
        if self.library is not None:
            _imports[self.library] = [imports.ReactImportVar(tag=None, render=False)]
...
class StatefulComponent(BaseComponent):
    ...
    def create(cls, component: Component) -> StatefulComponent | None:
        ...
        """
        from nextpy.interfaces.web.react_components.core.foreach import Foreach
    ...
    def _child_var(child: Component) -> Var | Component:
        ...
        """
        from nextpy.interfaces.web.react_components.base.bare import Bare
        from nextpy.interfaces.web.react_components.core.cond import Cond
        from nextpy.interfaces.web.react_components.core.foreach import Foreach
        ...
        for event_trigger, event_args in component._get_vars_from_event_triggers(
            ...
            memo_var_data = VarData.merge(
                *[var._var_data for var in event_args],
                VarData(  # type: ignore
                    imports={"react": {ReactImportVar(tag="useCallback")}},
                ),
            )
