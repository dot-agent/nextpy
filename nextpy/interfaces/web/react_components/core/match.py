...
from nextpy.interfaces.web.react_components.base import Fragment
from nextpy.interfaces.web.react_components.component import (
    BaseComponent,
    Component,
    MemoizationLeaf,
)
from nextpy.interfaces.web.react_components.tags import MatchTag, Tag
...
class Match(MemoizationLeaf):
    ...
    def _process_cases(
        ...
        if len([case for case in cases if not isinstance(case, tuple)]) > 1:
            raise ValueError("xt.match can only have one default case.")
    ...
    def _process_match_cases(cls, cases: List) -> List[List[BaseVar]]:
        ...
        for case in cases:
            if not isinstance(case, tuple):
                raise ValueError(
                    "xt.match should have tuples of cases and a default case as the last argument."
                )
