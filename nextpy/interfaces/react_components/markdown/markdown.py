...
from nextpy.interfaces.web.react_components.chakra.datadisplay.list import (
    ListItem,
    OrderedList,
    UnorderedList,
)
from nextpy.interfaces.web.react_components.chakra.navigation import Link
from nextpy.interfaces.web.react_components.chakra.typography.heading import Heading
from nextpy.interfaces.web.react_components.chakra.typography.text import Text
from nextpy.interfaces.web.react_components.component import Component, CustomComponent
...
def get_base_component_map() -> dict[str, Callable]:
    ...
    """
    from nextpy.interfaces.web.react_components.chakra.datadisplay.code import Code, CodeBlock
...
class Markdown(Component):
    ...
    def _get_imports(self) -> imports.ImportDict:
        ...
        imports.update(
            {
                "": [ReactImportVar(tag="katex/dist/katex.min.css")],
                "remark-math@5.1.1": [
                    ReactImportVar(tag=_REMARK_MATH._var_name, is_default=True)
                ],
                "remark-gfm@3.0.1": [
                    ReactImportVar(tag=_REMARK_GFM._var_name, is_default=True)
                ],
                "rehype-katex@6.0.3": [
                    ReactImportVar(tag=_REHYPE_KATEX._var_name, is_default=True)
                ],
                "rehype-raw@6.1.1": [
                    ReactImportVar(tag=_REHYPE_RAW._var_name, is_default=True)
                ],
            }
        )
