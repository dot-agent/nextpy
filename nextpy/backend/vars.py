...
def _encode_var(value: Var) -> str:
    ...
    """
    if value._var_data:
        return f"<nextpy.Var>{value._var_data.json()}</nextpy.Var>" + str(value)
    return str(value)
...
def _decode_var(value: str) -> tuple[VarData | None, str]:
    ...
    if isinstance(value, str):
        ...
        while m := re.match(r"(.*)<nextpy.Var>(.*)</nextpy.Var>(.*)", value):
...
class Var:
    ...
    def __bool__(self) -> bool:
        ...
        raise TypeError(
            ...
            "Instead use `xt.cond` and bitwise operators `&` (and), `|` (or), `~` (invert)."
        )
    ...
    def __iter__(self) -> Any:
        ...
        """
        raise TypeError(
            f"Cannot iterate over Var {self._var_full_name!r}. Instead use `xt.foreach`."
        )
                ...
                merge_var_data=VarData(
                    imports={
                        f"/{constants.Dirs.STATE_PATH}": [
                            ReactImportVar(tag="spreadArraysOrObjects")
                        ]
                    },
                ),
            )
                ...
                VarData(
                    imports={
                        "/utils/helpers/range.js": [
                            ReactImportVar(tag="range", is_default=True),
                        ],
                    },
                ),
            ),
    ...
    def as_ref(self) -> Var:
        ...
        return self._replace(
            ...
            merge_var_data=VarData(
                imports={
                    f"/{constants.Dirs.STATE_PATH}": [
                        imports.ReactImportVar(tag="refs")
                    ],
                },
            ),
        )
        ...
        new_var_data = VarData(
            ...
            imports={
                f"/{constants.Dirs.CONTEXTS_PATH}": [
                    ReactImportVar(tag="StateContexts")
                ],
