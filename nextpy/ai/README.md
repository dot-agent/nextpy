```diff
+ 🤖 'OpenAMS' provides basic utility functions for different agent utilities, inspired by CMS patterns. +
```
<p align="center"> 
...
### Here's the code for a full stack chat app with UI, all in a single Python file! (37 lines)
 ...
 ```python
import nextpy.ai.engine as engine
from nextpy.ai.engine._program import Log
from nextpy.ai import memory
...
@ui.on_chat_start
def start_chat():
    engine.llm = engine.llms.OpenAI(model="gpt-3.5-turbo")
