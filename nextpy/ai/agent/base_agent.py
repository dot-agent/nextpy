...
class BaseAgent:
    def __init__(
        ...
        if self.rag is not None and not self.input_variables.get("knowledge_variable"):
            raise ValueError(
    ...
    def llm_instance(self) -> engine.llms.OpenAI:
        ...
        return engine.llms.OpenAI(model=self.default_llm_model)
    ...
    def load_from_config(cls, config_file):
        ...
        if config["rag"] is not None:
            ...
            data_transformer_module_name, data_transformer_class_name = config["rag"][
                "data_transformer"
            ]["type"].rsplit(".", 1)
