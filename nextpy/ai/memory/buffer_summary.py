...
class BufferSummaryMemory(BaseMemory, BaseModel):
    ...
    def get_memory(self, **kwargs) -> str:
        ...
        new_messages = [
            ...
            if item not in self.messages_in_summary
            and item not in self.messages_in_buffer
        ]
        ...
        if len(new_messages) != 0:
            ...
            summary_messages = [
                item for item in self.messages if item not in buffer_messages
            ]  # summary
            ...
            if len(additions_to_summary) != 0:
                ...
                summarized_memory = summarizer(
                    summary=self.current_summary, new_lines=additions_to_summary_as_text
                )
                ...
                if len(new_messages) != 0:
                    ...
                    if len(additions_to_summary) != 0:
                        ...
                        summarized_memory = summarizer(
                            ...
                            new_lines=additions_to_summary_as_text,
                        )
                        self.current_summary = extract_text(summarized_memory.text)
                break
