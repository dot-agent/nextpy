...
comment = _SavedText(
    pp.Group(pp.Combine(comment_start + pp.ZeroOrMore(comment_content) + command_end))(
        "comment"
    )
)
...
long_comment_start = pp.Suppress(
...
long_comment = _SavedText(
    pp.Group(
        pp.Combine(
            long_comment_start + pp.ZeroOrMore(long_comment_content) + long_comment_end
        )
    )("long_comment").set_name("long_comment")
)
...
# basic literals
string_literal = pp.Group(
    pp.Suppress('"') + pp.ZeroOrMore(pp.CharsNotIn('"')) + pp.Suppress('"')
    | pp.Suppress("'") + pp.ZeroOrMore(pp.CharsNotIn("'")) + pp.Suppress("'")
)("string_literal")
...
multi_item_object = (
    object_start + object_item + pp.ZeroOrMore(object_sep + object_item) + object_end
)
object_literal <<= pp.Group(empty_object | single_item_object | multi_item_object)(
    "object_literal"
)
...
multi_item_array = (
    array_start + array_item + pp.ZeroOrMore(array_sep + array_item) + array_end
)
array_literal <<= pp.Group(empty_array | single_item_array | multi_item_array)(
    "array_literal"
)

literal <<= (
    string_literal | number_literal | boolean_literal | array_literal | object_literal
)
...
not_keyword = ~pp.FollowedBy(
    pp.Keyword("or") | pp.Keyword("else") | pp.Keyword("elif") | pp.Keyword("not")
)
command_name = pp.Combine(
    not_keyword + pp.Word(pp.srange("[@A-Za-z_]"), pp.srange("[A-Za-z_0-9\.]"))
)
variable_name = pp.Word(pp.srange("[@A-Za-z_]"), pp.srange("[A-Za-z_0-9]"))
variable_ref = not_keyword + pp.Group(
    pp.Word(pp.srange("[@A-Za-z_]"), pp.srange("[@A-Za-z_0-9\.\[\]\"'-]"))
)("variable_ref").set_name("variable_ref")
...
command_arg <<= pp.Group(named_command_arg)("named_command_arg").set_name(
    "named_command_arg"
) | pp.Group(code_chunk)("positional_command_arg").set_name("positional_command_arg")
...
ws_command_call <<= (
    command_name("name")
    + ~pp.FollowedBy(pp.one_of("+ - * / or not is and <= == >= != < >"))
    + ws_command_args
)
...
paren_command_call <<= (
    (command_name("name") + pp.Suppress("(")).leave_whitespace()
    - paren_command_args
    + pp.Suppress(")")
)
...
paren_group = (pp.Suppress("(") - enclosed_code_chunk + pp.Suppress(")")).set_name(
    "paren_group"
)
enclosed_code_chunk_cant_infix = (
    pp.Group(ws_command_call)("command_call")
    | pp.Group(paren_command_call)("command_call")
    | literal
    | keyword
    | variable_ref
    | paren_group
...
code_chunk_no_infix <<= (
    ...
    | keyword
    | variable_ref
)  # used by infix_operator_block
code_chunk_cant_infix = code_chunk_no_infix + ~pp.FollowedBy(
    pp.one_of("+ - * / or not is and <= == >= != < >")
)  # don't match infix operators so we can run this before infix_operator_block
...
simple_command_start = pp.Suppress(
    "{{" + ~pp.FollowedBy("!") + pp.Optional("~")
) + ~pp.FollowedBy(pp.one_of("# / >"))
command = _SavedText(
    pp.Group(simple_command_start + enclosed_code_chunk + command_end)("command")
)

# partial
always_call = pp.Group(
    paren_command_call | command_name("name") + pp.Optional(ws_command_args)
)
partial = pp.Group(
    pp.Suppress(pp.Combine(command_start + ">"))
    + always_call("command_call")
    + command_end
)("partial")

# block command {{#my_command arg1 arg2=val}}...{{/my_command}}
separator = pp.Group(
    pp.Keyword("or") | pp.Keyword("else") | (pp.Keyword("elif") + ws_command_args)
)("separator").set_name("separator")
block_command = pp.Forward()
block_command_call = always_call("command_call")
block_command_open = (
    pp.Suppress(pp.Combine(command_start + "#")) + block_command_call + command_end
)
block_command_sep = (command_start + separator + command_end)(
    "block_command_sep"
).set_name("block_command_sep")
block_command_close = _SavedText(
    pp.Group(command_start + pp.Suppress("/") + command_name + command_end)(
        "block_command_close"
    ).set_name("block_command_close")
)
block_command_content = (
    pp.Group(program)("block_content_chunk")
    + pp.ZeroOrMore(block_command_sep + pp.Group(program)("block_content_chunk"))
).set_name("block_content")
block_command <<= (
    block_command_open
    + _SavedText(pp.Group(block_command_content)("block_content"))
    + block_command_close
).leave_whitespace()
block_command = _SavedText(pp.Group(block_command)("block_command")).set_name(
    "block_command"
)
...
block_partial_open = (
    pp.Combine(command_start + pp.Suppress("#>")) + block_partial_call + command_end
)
...
escaped_command = _SavedText(
    pp.Group(
        pp.Suppress("\\")
        + command_start
        + pp.Combine(pp.ZeroOrMore(pp.CharsNotIn("}") | not_command_end))
        + command_end
    )("escaped_command")
)
...
program <<= (
    pp.ZeroOrMore(program_chunk)("program").leave_whitespace().set_name("program")
)
