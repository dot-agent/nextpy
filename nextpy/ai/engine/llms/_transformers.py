    ...
    def _model_and_tokenizer(self, model, tokenizer, **kwargs):
        ...
        if isinstance(model, str):
            ...
            try:
                ...
                raise Exception("Please install transformers with `pip install transformers` in order to use Compiler.llms.Transformers!")
...
class TransformersStreamer():
    ...
    def put(self, token_obj):
        ...
        if self.logprobs:
            assert len(new_tokens) == 1, "logprobs are not supported for batched generation right now in Compiler.llms.Transformers"
