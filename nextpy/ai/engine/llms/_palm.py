...
async def convert_to_engine_names_generator(chat_mode):
...
async def convert_to_engine_names_agenerator(chat_mode):
...
def convert_to_engine_names(chat_mode):
    if isinstance(chat_mode, (palm.TextGenerationResponse)):
        ...
        return convert_to_engine_names_agenerator(chat_mode)
    elif isinstance(chat_mode, (types.GeneratorType)):
        return convert_to_engine_names_generator(chat_mode)
    else:
...
class PaLM(LLM):
    ...
    async def _library_call(self, **kwargs):
        ...
        return convert_to_engine_names(out)
    ...
    async def _rest_call(self, **kwargs):
        ...
        response = convert_to_engine_names(response)
...
class PALMSession(LLMSession):
    async def __call__(
        ...
        **completion_kwargs,
        ...
        assert (
            not pattern
        ), "The PaLM API does not support Engine pattern controls! Please either switch to an endpoint that does, or don't use the `pattern` argument to `gen`."
