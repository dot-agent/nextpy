...
#     end_string = "<|im_end|>"
#     start_position = string.find(start_string)
#     end_position = string.find(end_string, start_position)
...
#         prompt_type = string[start_position + len(start_string) : split]
#         text = string[split + 1 : end_position]
#         output.append([prompt_type,text])

#         # Remove used text from the string
#         string = string.replace(start_string, "", 1)
#         string = string.replace(end_string, "", 1)
...
#         new_output.append({user[1]: llm[1]})


#     return new_output

# input: prompt template without conversation history variable
# output: same prompt template with conversation history variable appended to the end of the system prompt
# def add_variable(string):
#     formated_string = string.replace("{{~/system}}","{{ ConversationHistory }}{{~/system}}")
    ...
    def __str__(self) -> str:
        ...
        for entry in self._entries:
            ...
            string_entries.append(
                f"{entry.get('time', '')}{type_str: >10s}{entry.get('name', ''): >15s}: "
                + str(
                    {
                        k: v
...
class Program:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        /!Removing memory for now to make it compatable with nextpy will add it back later/ 
        Memory: None or Memory Instance
            if None, the program will not utilize memory
            if Memory Class Instance is passed, it will be used to store and retrieve conversations
        """
        ...
        self._variables.update({"llm": llm})
        ...
        self._last_display_update = (
            0  # the last time we updated the display (used for throttling updates)
        )
        self._execute_complete = (
            asyncio.Event()
        )  # fires when the program is done executing to resolve __await__
        self._emit_stream_event = (
            asyncio.Event()
        )  # fires when we need to emit a stream event
        self._exception = (
            None  # if the program finished with an exception its stored here
        )
        self._displaying = (
            not self.silent
        )  # if we are displaying we need to update the display as we execute
        ...
        if os.environ.get("VSCODE_CWD", None) is not None:
            ...
            self.display_throttle_limit = (
                0.1  # the minimum time between display updates
            )
        self.update_display = DisplayThrottler(
            self._update_display, self.display_throttle_limit
        )
    ...
    def _ipython_display_(self):
        ...
        log.debug(
            f"displaying program in _ipython_display_ with self._comm={self._comm}, self.id={self._id}"
        )
    ...
    def __call__(self, **kwargs):
        ...
        new_program = Program(
            ...
            **{
                **{
                    k: v if callable(v) else copy.deepcopy(v)
        ...
        if new_program.async_mode:
            ...
            update_task = loop.create_task(
                new_program.update_display.run()
            )  # start the display updater
            execute_task = loop.create_task(
                # removing agent and memory 
                # new_program.execute(memory=self.memory, from_agent=from_agent)
                new_program.execute()
            )
        ...
        while new_program._executor is not None:
            try:
                loop.run_until_complete(
                    execute_task
                )  # this will stop each time the program wants to emit a new state
    ...
    def _update_display(self, last=False):
        ...
        log.debug(
            f"Updating display (last={last}, self._displaying={self._displaying}, self._comm={self._comm})"
        )
        ...
        if self._displaying_html:
            ...
            if (
                self._displayed and self._comm and self._comm.is_open
            ):  # (not last or self._comm.is_open):
            ...
            # ...otherwise dump the client to the front end
                ...
                if self._displayed:
                    clear_output(
                        wait=True
                    )  # TODO: should use wait=True but that doesn't work in VSCode until after the April 2023 release
    ...
    def _display_html(self, html):
        ...
        if self._comm is None:
            self._comm = _utils.JupyterComm(
                self._id, self._ipython, self._interface_event
            )
...
<script type="text/javascript">{js_data}; window._engineDisplay("{self._id}");</script>"""
    ...
    async def execute(self):
        ...
        log.debug(
            f"Executing program (self.async_mode={self.async_mode}, self.silent={self.silent}, self._displaying_html={self._displaying_html})"
        )
    ...
    def _build_html(self, text, last=False):
        ...
        def start_generate_or_select(x):
            ...
            if no_echo:
                ...
                out += "<span style='background-color: rgba(0, 165, 0, 0.25); opacity: {}; display: none;' title='{}'>".format(
                    alpha, undo_html_encode(x.group(1))
                )
            else:
                out = "<span style='background-color: rgba(0, 165, 0, 0.25); opacity: {}; display: inline;' title='{}'>".format(
                    alpha, undo_html_encode(x.group(1))
                )
            return out
        ...
        def start_each(x):
            ...
            return "<span style='opacity: {}; display: inline; background-color: {};' title='{}'>".format(
                alpha, color, undo_html_encode(x.group(1))
            )
        ...
        def role_box(x):
            ...
            start_pattern = html.escape(self.llm.role_start(role_name)).replace(
                "|", r"\|"
            )
            start_pattern_with_name = html.escape(
                self.llm.role_start(role_name, __ARxG__="__VAxLUE__")
            ).replace(
                "|", r"\|"
            )  # TODO: make this more general for multiple keyword args
            start_pattern_with_name = start_pattern_with_name.replace(
                "__VAxLUE__", "[^\n]*?"
            ).replace("__ARxG__", "[^=]*?")
            ...
            content = re.sub(
                "^" + start_pattern_with_name, "", content, flags=re.DOTALL
            )
        ...
        display_out = re.sub(
            r"{{!--GMARKER_START[^}]*--}}{{!--GHIDDEN:(.*?)--}}{{!--GMARKER_END[^}]*--}}",
            "",
            display_out,
            flags=re.DOTALL,
        )
        ...
        display_out = re.sub(
            r"(\{\{(?!\!)(?!~\!).*?\}\})",
            r"<span style='font-family: monospace; background-color: rgba(0, 0, 0, 0.05);'>\1</span>",
            display_out,
            flags=re.DOTALL,
        )
        ...
        if (
            re.search(
                r"{{!--GMARKER_START_(role|system|user|assistant|function|context|example)",
                display_out,
            )
            is not None
        ):
            ...
            display_out = re.sub(
                r"\s*{{!--GMARKER_START_(role|system|user|assistant|function|context|example)\$(.*?)--}}",
                r"{{!--GMARKER_START_\1$\2--}}",
                display_out,
                flags=re.DOTALL,
            )
            ...
            display_out = re.sub(
                r"{{!--GMARKER_END_(role|system|user|assistant|function|context|example)\$(.*?)--}}\s*",
                r"{{!--GMARKER_END_\1$\2--}}",
                display_out,
                flags=re.DOTALL,
            )
            ...
            display_out = re.sub(
                r"{{!--GMARKER_START_(role|system|user|assistant|function|context|example)\$(.*?)--}}"
                + "(.*?)"
                + r"{{!--GMARKER_END_(role|system|user|assistant|function|context|example)\$(.*?)--}}",
                role_box,
                display_out,
                flags=re.DOTALL,
            )
            ...
            display_out = re.sub(
                r"{{!--GMARKER_START_(input|output)\$(.*?)--}}"
                + "(.*?)"
                + r"{{!--GMARKER_END_(input|output)\$(.*?)--}}",
                input_output_box,
                display_out,
                flags=re.DOTALL,
            )
        ...
        display_out = re.sub(
            r"(\{\{generate.*?\}\})",
            r"<span style='background-color: rgba(0, 165, 0, 0.25);'>\1</span>",
            display_out,
            flags=re.DOTALL,
        )
        display_out = re.sub(
        ...
        display_out = re.sub(
            r"(\{\{#each [^'\"].*?\{\{/each.*?\}\})",
            r"<span style='background-color: rgba(0, 138.56128016, 250.76166089, 0.25);'>\1</span>",
            display_out,
            flags=re.DOTALL,
        )
...
cycle_IDVAL(this);""".replace(
        ...
        def click_loop_mid(id, index, echo):
            ...
            out = (
                f"</div><div style='display: none; opacity: {alpha}' id='{id}_{index}'>"
            )
            return out

        display_out = re.sub(
            r"{{!--GMARKERmany_generate_start_([^_]+)_([0-9]+)\$([^\$]*)\$--}}",
            lambda x: click_loop_start(
                x.group(3),
                int(x.group(2)),
                x.group(1) == "True",
                "rgba(0, 165, 0, 0.25)",
        ...
        display_out = re.sub(
            r"--}}{{!--GMARKERmany_generate_end\$([^\$]*)\$--}}", "</div>", display_out
        )

        # format the each command results
        display_out = re.sub(
            r"{{!--GMARKER_START_each\$([^\$]*)\$--}}", start_each, display_out
        )
        display_out = re.sub(
            r"{{!--GMARKER_each_noecho_start_([^_]+)_([0-9]+)\$([^\$]*)\$--}}",
            lambda x: click_loop_start(
                x.group(3), int(x.group(2)), False, "rgb(100, 100, 100, 1)"
            ),
            display_out,
        )
        display_out = re.sub(
        ...
        display_out = re.sub(
            r"{{!--GMARKER_each_noecho_end\$([^\$]*)\$--}}", "</div>", display_out
        )

        # format the geneach command results
        display_out = re.sub(
            r"{{!--GMARKER_START_geneach\$([^\$]*)\$--}}", start_each, display_out
        )
        ...
        display_out = re.sub(
            r"{{!--GMARKER_set\$([^\$]*)\$--}}", r"", display_out
        )  # just hide them for now

        display_out = re.sub(
            r"{{!--GMARKER_START_select\$([^\$]*)\$--}}",
            start_generate_or_select,
            display_out,
        )
        display_out = display_out.replace("{{!--GMARKER_END_select$$--}}", "</span>")
        display_out = re.sub(
            r"{{!--GMARKER_START_variable_ref\$([^\$]*)\$--}}",
            lambda x: "<span style='background-color: rgba(0, 138.56128016, 250.76166089, 0.25); display: inline;' title='{}'>".format(
                undo_html_encode(x.group(1))
            ),
            display_out,
        )
        display_out = display_out.replace(
            "{{!--GMARKER_END_variable_ref$$--}}", "</span>"
        )
        display_out = display_out.replace(
            "{{!--GMARKER_each$$--}}", ""
        ...
        display_out = re.sub(
            r"{{!--GMARKER_START_block\$([^\$]*)\$--}}", start_block, display_out
        )
        display_out = re.sub(
            r"{{!--GMARKER_START_([^\$]*)\$([^\$]*)\$--}}",
            lambda x: "<span style='background-color: rgba(0, 138.56128016, 250.76166089, 0.25); display: inline;' title='{}'>".format(
                undo_html_encode(x.group(2))
            ),
            display_out,
        )
        display_out = re.sub(
            r"{{!--GMARKER_END_([^\$]*)\$\$--}}", "</span>", display_out
        )
...
class DisplayThrottler:
    ...
    async def run(self):
        ...
        while True:
            ...
            now = time.time()
            log.info(
                "in DisplayThrottler run loop -- now: {}, last_time: {}, throttle_limit: {}".format(
                    now, self.last_time, self.throttle_limit
                )
            )
