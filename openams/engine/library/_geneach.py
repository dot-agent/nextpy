...
async def geneach(
    ...
    join="",
    single_call=False,
    single_call_temperature=0.0,
    single_call_max_tokens=500,
    single_call_top_p=1.0,
    ...
    if not single_call:
        for i in range(max_iterations):
            ...
            with ContentCapture(variable_stack, hidden) as new_content:
                ...
                variable_stack.push({"@index": i, "@first": i == 0, "this": {}})
                ...
                variable_stack[list_name] = variable_stack.get(list_name, []) + [
                    block_variables
                ]
            ...
            if stop is not False and i >= min_iterations and i < max_iterations:
                try:
                    gen_obj = await parser.llm_session(
                        variable_stack["@prefix"],
                        stop=stop,
                        max_tokens=max_stop_tokens,
                        temperature=0,
                        cache_seed=0,
                    )
                except Exception:
                    raise Exception(
                        f"Error generating stop tokens for geneach loop. Perhaps you are outside of role tags (assistant/user/system/function)? If you don't want the loop to check for stop tokens, set stop=False or set num_iterations."
                    )
    ...
    # TODO: right now single_call is a bit hacky, we should make it more robust to rich loop item template structures
        ...
        if min_iterations > 0:
            ...
            fixed_prefix = fixed_prefix.replace(
                r"{{@index}}", "0"
        ...
        if single_call_temperature > 0:
            ...
            cache_seed = 0
        gen_stream = await parser.llm_session(
            variable_stack["@raw_prefix"] + fixed_prefix,
            stop=stop,
            max_tokens=single_call_max_tokens,
            temperature=single_call_temperature,
            top_p=single_call_top_p,
        ...
        for gen_obj in gen_stream:
            ...
            for (
                ...
                matches
            ):  # "{{!--" + f"GMARKER_START_{name}${node_text}$}}{out}{{!--GMARKER_END_{name}$$" + "}}"
                ...
                item_out = re.sub(
                    r"{{gen [\'\"]([^\'\"]+)[\'\"][^}]*}}",
                    lambda x: "{{!--GMARKER_START_gen$"
                    + x.group()
                    .replace("$", "&#36;")
                    .replace("{", "&#123;")
                    .replace("}", "&#125;")
                    + "$--}}"
                    + match_dict[_escape_group_name(x.group(1))]
                    + "{{!--GMARKER_END_gen$$--}}",
                    item_out,
                )
                variable_stack["@raw_prefix"] += (
                    "{{!--GMARKER_each$$--}}" + item_out
                )  # marker and content of the item
                num_items += 1
                # out.append(item_out)
