...
class Transformers(LLM):
    ...
    def role_start(role):
        raise NotImplementedError(
            "In order to use chat role tags you need to use a chat-specific subclass of Transformers for your LLM from engine.transformers.*!"
        )
    ...
    def _model_and_tokenizer(self, model, tokenizer, **kwargs):
        ...
        if isinstance(model, str):
            ...
            try:
                ...
                raise Exception(
                    "Please install transformers with `pip install transformers` in order to use engine.endpoints.Transformers!"
                )
        ...
        ), "You must give a tokenizer object when you provide a model object (as opposed to just a model name)!"
    ...
    async def __call__(
        ...
        **generate_kwargs
        ...
        if "function_call" in generate_kwargs:
            assert generate_kwargs["function_call"] in [
                "none"
            ], "Transformers does not yet have function call support!"
            ...
            for obj in [model_config, self.llm.tokenizer]:
                for attr in possible_attributes:
                    if max_context is None:
                        ...
                        break
            assert max_context is not None, (
                "Could not find a max context length for the model! Tried: "
...
class RegexStoppingCriteria:
    ...
    def __call__(self, input_ids, scores, **kwargs):
        ...
        if self.current_strings is None:
            self.current_strings = [
                self.llm.new_string_builder() for _ in range(len(input_ids))
            ]
        for i in range(len(self.current_strings)):
            self.current_strings[i].extend(input_ids[i][self.current_length :])
...
class TransformersStreamer:
    def __init__(
        ...
        self.generated_string = [
            self.llm.new_string_builder(input_ids[0])
    ...
    def put(self, token_obj):
        ...
        if self.logprobs:
            assert (
                len(new_tokens) == 1
            ), "logprobs are not supported for batched generation right now in engine.endpoints.Transformers"
            new_scores = [
                torch.nn.functional.log_softmax(x, dim=-1).cpu()
                for x in token_obj["scores"]
            ]
        ...
        for i in range(len(self.input_ids)):
            ...
            if self.logprobs:
                for scores in new_scores[i]:
                    if scores is None:
                        ...
                        top_inds = scores[0].argsort(descending=True)[
                            : self.logprobs
                        ]  # TODO: verify the [0] is always correct
                        self.display_logprobs[i].append(
                            {
                                self.llm.id_to_token(j): float(scores[0][j])
                                for j in top_inds
                            }
                        )
            ...
            if self.sequence_pos[i] < len(self.generated_sequence[i]):
                display_tokens = list(
                    self.generated_sequence[i][self.sequence_pos[i] :]
                )
                ...
                if self.str_pos[i] < len(self.generated_string[i]):
                    ...
                    if len(self.generated_sequence[i]) >= self.max_total_tokens:
                        ...
                        self.generated_sequence[i][-1]
                        == self.llm.tokenizer.eos_token_id
                    ):
                        finish_reason = "endoftext"
                        eos_str = self.generated_string[
                            i
                        ].pop()  # remove the end of text token
                    ...
                    stop_text = None
                    if (
                        self.stop_regex is not None
                    ):  # and (finish_reason is None or len(self.input_ids) > 1):
                    ...
                    if not found_partial or finish_reason is not None:
                        out["choices"][i] = {
                            ...
                            "logprobs": {
                                ...
                                "top_logprobs": self.display_logprobs[i][
                                    self.sequence_pos[i] :
                                ]
                            },
                        }
    ...
    def end(self):
        ...
        for i in range(len(self.input_ids)):
            assert self.str_pos[i] >= len(
                self.generated_string[i]
            ), "Not all data was flushed, this means generation stopped for an unknown reason!"
