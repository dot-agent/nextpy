...
from openams.audio.text_to_speech.base import TextToSpeech
...
def get_text_to_speech(tts: str = None) -> TextToSpeech:
    ...
    if tts == 'ELEVEN_LABS':
        from openams.audio.text_to_speech.elevenlabs import ElevenLabs
        ElevenLabs.initialize()
        return ElevenLabs.get_instance()
    elif tts == 'GOOGLE_TTS':
        from openams.audio.text_to_speech.google_cloud_tts import GoogleCloudTTS
        GoogleCloudTTS.initialize()
        return GoogleCloudTTS.get_instance()
    elif tts == 'UNREAL_SPEECH':
        from openams.audio.text_to_speech.unreal_speech import UnrealSpeech
