...
from nextpy.backend.deploy.hosting import DeploymentPrepInfo
from nextpy.cli import cli
...
def test_login_success_existing_token(mocker):
    mocker.patch(
        "nextpy.backend.deploy.hosting.authenticated_token",
...
def test_login_success_on_browser(mocker):
    ...
    mock_authenticate_on_browser = mocker.patch(
        "nextpy.backend.deploy.hosting.authenticate_on_browser",
...
def test_login_fail(mocker):
    ...
    mocker.patch(
        "nextpy.backend.deploy.hosting.get_existing_access_token", return_value=("", "")
    )
    mocker.patch(
        "nextpy.backend.deploy.hosting.authenticate_on_browser", return_value=""
    )
    result = runner.invoke(cli, ["login"])
...
def setup_env_authentication(mocker):
    mocker.patch("nextpy.build.prerequisites.check_initialized")
    mocker.patch("nextpy.build.dependency.check_requirements")
    mocker.patch(
        "nextpy.backend.deploy.hosting.authenticated_token", return_value="fake-token"
    )
...
def test_deploy_non_interactive_prepare_failed(
    ...
    mocker.patch(
        "nextpy.backend.deploy.hosting.prepare_deploy",
...
def test_deploy_non_interactive_success(
    ...
    mocker.patch("nextpy.utils.console.ask")
    ...
    mocker.patch("nextpy.cli.export")
    mock_deploy = mocker.patch(
        "nextpy.backend.deploy.hosting.deploy",
        return_value=Mock(
            frontend_url="fake-frontend-url", backend_url="fake-backend-url"
        ),
    )
    mocker.patch("nextpy.backend.deploy.hosting.wait_for_server_to_pick_up_request")
    mocker.patch("nextpy.backend.deploy.hosting.poll_deploy_milestones")
    mocker.patch("nextpy.backend.deploy.hosting.poll_backend", return_value=True)
    mocker.patch("nextpy.backend.deploy.hosting.poll_frontend", return_value=True)
...
def test_deploy_interactive(
    ...
    mocker.patch(
        "nextpy.backend.deploy.hosting.check_requirements_for_non_nextpy_packages",
        return_value=True,
    )
    mocker.patch(
        "nextpy.backend.deploy.hosting.prepare_deploy",
    ...
    mocker.patch(
        "nextpy.backend.deploy.hosting.interactive_get_deployment_key_from_user_input",
        return_value=(expected_key, "fake-api-url", "fake-deploy-url"),
    )
    mocker.patch("nextpy.utils.console.ask", side_effect=user_input_region)
    mocker.patch(
        "nextpy.backend.deploy.hosting.interactive_prompt_for_envs",
        return_value=user_input_envs,
    )
    fake_export_dir = "fake-export-dir"
    mocker.patch("tempfile.mkdtemp", return_value=fake_export_dir)
    mocker.patch("nextpy.cli.export")
