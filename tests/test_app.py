...
import nextpy.frontend.components.radix.themes as rdxt
...
from nextpy.backend.middleware import HydrateMiddleware
...
from nextpy.frontend.components import Box, Component, Cond, Fragment, Text
...
def test_multiple_states_error(monkeypatch, test_state, redundant_test_state):
    """Test that an error is thrown when multiple classes subclass xt.BaseState.

    Args:
        monkeypatch: Pytest monkeypatch object.
        test_state: A test state subclassing xt.BaseState.
        redundant_test_state: Another test state subclassing xt.BaseState.
    """
...
async def test_upload_file(tmp_path, state, delta, token: str, mocker):
    ...
    """
    mocker.patch(
        "nextpy.state.State.class_subclasses",
    ...
    request_mock.headers = {
        "nextpy-client-token": token,
        "nextpy-event-handler": f"state.{state_name}.multi_handle_upload",
    }
...
async def test_upload_file_without_annotation(state, tmp_path, token):
    ...
    request_mock.headers = {
        "nextpy-client-token": token,
        "nextpy-event-handler": f"{state_name}.handle_upload2",
    }
...
async def test_upload_file_background(state, tmp_path, token):
    ...
    request_mock.headers = {
        "nextpy-client-token": token,
        "nextpy-event-handler": f"{state_name}.bg_upload",
    }
    file_mock = unittest.mock.Mock(filename="image1.jpg")
    fn = upload(app)
    ...
    assert (
        err.value.args[0]
        == f"@xt.background is not supported for upload handler `{state_name}.bg_upload`."
    )
