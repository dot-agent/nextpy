...
from nextpy.backend.event import Event, EventHandler
...
from nextpy.backend.vars import BaseVar, ComputedVar
...
async def test_state_with_invalid_yield(capsys):
    ...
    async for update in invalid_state._process(
        ...
        assert update.events == xt.event.fix_events(
            [xt.window_alert("An error occurred. See logs for details.")],
            token="",
        )
...
def test_duplicate_substate_class(mocker):
    mocker.patch("nextpy.state.os.environ", {})
    with pytest.raises(ValueError):
...
async def test_preprocess(app_module_mock, token, test_state, expected, mocker):
    ...
    """
    mocker.patch("nextpy.state.State.class_subclasses", {test_state})
    ...
    updates = []
    async for update in xt.app.process(
        app=app,
        event=Event(
            token=token,
...
async def test_preprocess_multiple_load_events(app_module_mock, token, mocker):
    ...
    Args:
        ...
        mocker: pytest mock object.
    """
    mocker.patch("nextpy.state.State.class_subclasses", {OnLoadState})
    app = app_module_mock.app = App(
        state=State,
