...
def test_event_namespace(mocker, kwargs, expected):
    ...
    mocker.patch("nextpy.build.config.get_config", return_value=conf)
...
def test_replace_defaults(
    ...
    monkeypatch.setattr(nextpy.build.config.os, "environ", mock_os_env)  # type: ignore
