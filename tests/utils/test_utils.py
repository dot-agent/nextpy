...
def test_validate_invalid_bun_path(mocker):
    ...
    mocker.patch("nextpy.build.prerequisites.get_config", mock)
    mocker.patch("nextpy.build.prerequisites.get_bun_version", return_value=None)
...
def test_validate_bun_path_incompatible_version(mocker):
    ...
    mocker.patch("nextpy.build.prerequisites.get_config", mock)
    mocker.patch(
        "nextpy.build.prerequisites.get_bun_version",
...
def test_remove_existing_bun_installation(mocker):
    ...
    """
    mocker.patch("nextpy.build.prerequisites.os.path.exists", return_value=True)
    rm = mocker.patch("nextpy.build.prerequisites.path_ops.rm", mocker.Mock())
...
def test_setup_frontend(tmp_path, mocker):
    ...
    mocker.patch("nextpy.build.prerequisites.install_frontend_packages")
    mocker.patch("nextpy.build.build.set_env_json")
...
def test_create_config(app_name, expected_config_name, mocker):
    ...
    tmpl_mock = mocker.patch("nextpy.build.compiler.templates.XTCONFIG")
...
def test_app_default_name(tmp_path, mocker):
    ...
    mocker.patch("nextpy.build.prerequisites.os.getcwd", return_value=str(nextpy))
...
def test_node_install_windows(tmp_path, mocker):
    ...
    mocker.patch("nextpy.build.prerequisites.constants.Fnm.DIR", fnm_root_path)
    mocker.patch("nextpy.build.prerequisites.constants.Fnm.EXE", fnm_exe)
    mocker.patch("nextpy.build.prerequisites.constants.IS_WINDOWS", True)
    mocker.patch("nextpy.build.processes.new_process")
    mocker.patch("nextpy.build.processes.stream_logs")
    ...
    download = mocker.patch("nextpy.build.prerequisites.download_and_extract_fnm_zip")
    mocker.patch("nextpy.build.prerequisites.zipfile.ZipFile")
    mocker.patch("nextpy.build.prerequisites.path_ops.rm")
...
def test_node_install_unix(tmp_path, mocker, machine, system):
    ...
    mocker.patch("nextpy.build.prerequisites.constants.Fnm.DIR", fnm_root_path)
    mocker.patch("nextpy.build.prerequisites.constants.Fnm.EXE", fnm_exe)
    mocker.patch("nextpy.build.prerequisites.constants.IS_WINDOWS", False)
    mocker.patch("nextpy.build.prerequisites.platform.machine", return_value=machine)
    mocker.patch("nextpy.build.prerequisites.platform.system", return_value=system)
    ...
    mocker.patch("httpx.stream", return_value=Resp())
    download = mocker.patch("nextpy.build.prerequisites.download_and_extract_fnm_zip")
    process = mocker.patch("nextpy.build.processes.new_process")
    chmod = mocker.patch("nextpy.build.prerequisites.os.chmod")
    mocker.patch("nextpy.build.processes.stream_logs")
...
def test_bun_install_version(mocker, bun_version):
    """Test that bun is downloaded when the host version(installed by nextpy)
    different from the current version set in nextpy.
    ...
    mocker.patch("nextpy.utils.path_ops.which")
    mock = mocker.patch("nextpy.build.prerequisites.download_and_run")
...
def test_create_nextpy_dir(mocker, is_windows):
    ...
    """
    mocker.patch("nextpy.build.prerequisites.constants.IS_WINDOWS", is_windows)
    mocker.patch("nextpy.build.prerequisites.processes.run_concurrently", mocker.Mock())
    mocker.patch("nextpy.build.prerequisites.initialize_web_directory", mocker.Mock())
    mocker.patch("nextpy.build.processes.run_concurrently")
    mocker.patch("nextpy.build.prerequisites.validate_bun")
    create_cmd = mocker.patch(
        "nextpy.build.prerequisites.path_ops.mkdir", mocker.Mock()
    )
