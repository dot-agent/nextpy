...
def test_authenticated_token_success(mocker):
    ...
    invitation_code = "fake_code"
    mocker.patch(
        "nextpy.backend.deploy.hosting.get_existing_access_token",
        return_value=(access_token, invitation_code),
    )
    mocker.patch(
        "nextpy.backend.deploy.hosting.validate_token_with_retries", return_value=True
    )
...
def test_maybe_authenticated_token_is_invalid(mocker):
    ...
    mocker.patch(
        "nextpy.backend.deploy.hosting.validate_token_with_retries", return_value=False
    )
    assert hosting.authenticated_token()[0] == ""
...
def test_prepare_deploy_not_authenticated(mocker):
    mocker.patch(
        "nextpy.backend.deploy.hosting.requires_authenticated", return_value=None
    )
...
def test_server_unable_to_prepare_deploy(mocker):
    mocker.patch(
        "nextpy.backend.deploy.hosting.requires_authenticated",
...
def test_deploy(mocker):
    mocker.patch(
        "nextpy.backend.deploy.hosting.requires_access_token", return_value="fake_token"
    )
    mocker.patch("builtins.open")
    mocker.patch(
        "httpx.post",
...
def test_validate_token_with_retries_failed(mocker):
    mock_validate_token = mocker.patch(
        "nextpy.backend.deploy.hosting.validate_token", side_effect=Exception
    )
    mock_delete_token = mocker.patch(
        "nextpy.backend.deploy.hosting.delete_token_from_config"
    )
...
def test_validate_token_with_retries_access_denied(mocker):
    mock_validate_token = mocker.patch(
        "nextpy.backend.deploy.hosting.validate_token", side_effect=ValueError
    )
    mock_delete_token = mocker.patch(
        "nextpy.backend.deploy.hosting.delete_token_from_config"
    )
...
def test_validate_token_with_retries_success(mocker):
    ...
    mock_validate_token = mocker.patch(
        "nextpy.backend.deploy.hosting.validate_token",
        side_effect=validate_token_returns,
    )
    mock_delete_token = mocker.patch(
        "nextpy.backend.deploy.hosting.delete_token_from_config"
    )
...
def test_requirements_txt_only_contains_nextpy(mocker):
    mocker.patch(
        "nextpy.backend.deploy.hosting.check_requirements_txt_exist", return_value=True
    )
    mocker.patch("builtins.open", mock_open(read_data="\nnextpy=1.2.3\n\n"))
