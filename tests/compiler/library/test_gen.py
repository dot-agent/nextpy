...
def test_gen():
    ...
    llm = engine.endpoints.Mock(" Sue")
    prompt = engine("Hello my name is{{gen 'name' max_tokens=5}}", llm=llm)
    out = prompt()
    assert len(out["name"]) > 1

def test_gen_n_greater_than_one():
    llm = engine.endpoints.Mock(["mock output 0", "mock output 1", "mock output 2"])
    prompt = engine('''The best thing about the beach is{{gen 'best' n=3 temperature=0.7 max_tokens=5}}''', llm=llm)
    a = prompt()
...
def test_gen_n_greater_than_one_hidden():
    ...
    prompt = engine('''The best thing about the beach is{{gen 'best' temperature=0.7 n=3 hidden=True}}
...
def test_pattern():
    ...
    out = engine('''On a scale of 1-10 I would say it is: {{gen 'score' pattern="[0-9]+"}}''', llm=llm)()
...
def test_custom_kwargs_transformers(llm):
    ...
    llm = get_llm(llm)
    program = engine('''Repeat the following 10 times: Repeat this. Repeat this. Repeat this. Repeat this.{{gen 'completion' max_tokens=4 repetition_penalty=10.0}}''', llm=llm)
    executed_program = program()
...
def test_stop(llm):
    ...
    llm = get_llm(llm)
    program = engine("""Write "repeat this. " 10 times: repeat this. repeat this. repeat this. repeat this. repeat this. repeat this.{{gen stop="this" max_tokens=10}}""", llm=llm)
    out = program()
...
def test_stop_regex(llm):
    ...
    llm = get_llm(llm)
    program = engine("""Write "repeat this. " 10 times: repeat this. repeat this. repeat this. repeat this. repeat this. repeat this.{{gen stop_regex="th.s" max_tokens=10}}""", llm=llm)
    out = program()
    assert str(out) == "Write \"repeat this. \" 10 times: repeat this. repeat this. repeat this. repeat this. repeat this. repeat this. repeat "
...
def test_save_stop_text(llm):
    llm = get_llm(llm)
    out = engine("""Repeat this ten times: "s38 kdjksid sk slk", "s38 kdjksid sk slk", "s38 kdjksid sk slk", "s38 kdjksid sk slk", "{{gen 'text' stop_regex="kdj.*slk" max_tokens=10 save_stop_text=True}}""", llm=llm)()
...
def test_stop_regex_cut_short(llm):
    ...
    llm = get_llm(llm)
    out = engine("""Repeat this ten times: "s38 kdjksid", "s38 kdjksid", "s38 kdjksid", "s38 kdjksid", "{{gen 'text' stop_regex="s38 kdjksid" max_tokens=5 save_stop_text=True}}""", llm=llm)()
...
def test_gen_stream(llm):
    ...
    prompt = engine("Hello my name is{{gen 'name' max_tokens=10 stream=True}}", llm=llm1)
    out = prompt()
    assert len(out["name"]) > 1

    # make sure it also works with caching
    llm2 = get_llm(llm, caching=True)
    prompt = engine("Hello my name is{{gen 'name' max_tokens=10 stream=True}}", llm=llm2)
    out = prompt()
    assert len(out["name"]) > 1