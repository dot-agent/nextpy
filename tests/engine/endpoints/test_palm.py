...
def test_context_palm():
    ...
    """
    engine.llm = get_llm("palm:chat-bison")
...
def test_examples_palm(rest_call):
    engine.llm = get_llm("palm:chat-bison", rest_call=rest_call)
    chat = engine(
        """
{{~#context~}}
...
def test_examples_palm_32k():
    engine.llm = get_llm("palm:chat-bison-32k", rest_call=True)
    chat = engine(
        """
{{~#context~}}
You always feel the same
...
def test_nostream():
    engine.llm = get_llm("palm:text-bison")
    a = engine(
...
def test_rest_nostream():
    engine.llm = get_llm("palm:text-bison", rest_call=True)
    a = engine(
        """What's your name? {{gen 'name' stream=False max_tokens=5}}""", stream=False
    )
    a = a()
...
def test_rest_stream_chat():
    ...
    async def f():
        engine.llm = get_llm("palm:chat-bison", rest_call=True)
        chat = engine(
            """
{{~#user~}}
