...
def test_each():
    ...
    assert (
        str(prompt(name="Engine", names=["Bob", "Sue"])) == "Hello, Engine! Bob Sue"
    )
...
def test_each_with_objects():
    ...
    out = prompt(name="Engine", names=[{"name": "Bob"}, {"name": "Sue"}])
...
def test_each_after_await():
    ...
    assert (
        str(prompt(name="Engine", names=["Bob", "Sue"]))
        == "Hello, Engine!{{await 'some_var'}}{{#each names}} {{this}}{{/each}}"
    )
...
def test_each_over_an_await():
    ...
    assert (
        ...
        == "Hello, Engine!{{#each (await 'names')}} {{this}}{{/each}}"
    )
...
{{/each}}""",
    ...
    assert (
        ...
        == "Hello, Engine! Here are 5 names and their favorite food:\n"
    )
